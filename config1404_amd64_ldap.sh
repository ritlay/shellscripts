...
PADRAO="aptitude build-essential ldap-auth-client nscd openssh-server vim codeblocks chromium-browser gnome-panel p7zip-full traceroute qemu-kvm libvirt-bin ubuntu-vm-builder bridge-utils lib32bz2-1.0 lib32ncurses5-dev lib32stdc++6 lib32z1 libxml-simple-perl perl libio-compress-perl libdbi-perl libdbd-mysql-perl libapache-dbi-perl perl libnet-ip-perl libsoap-lite-perl libphp-pclzip libxml-parser-perl ubuntu-restricted-extras davfs2 curl blender git frama-c compizconfig-settings-manager nfs-common tcl8.5 tk8.5 owncloud-client-nautilus"
PACOTES="vlc wireshark haskell-platform geany postgresql pgadmin3 fpc qtoctave qtcreator grace oracle-java8-installer oracle-java8-set-default astah-community sublime-text atom ubuntu-make android-tools-adb"
...
echo -e '\n\nScript de configuração inicial para Ubuntu 14.04 AMD64\n\n'
...
if [ -e /etc/apt/sources.list.backup ];  then
  echo -e '\nArquivo de backup "sources.list.backup" ja existe\n';sleep 1
    ...
    echo "deb http://br.archive.ubuntu.com/ubuntu/ trusty main restricted
...
# Astah    
    if [ -e /etc/apt/sources.list.d/astah.list ];  then
    	echo 'Repositorio Astah ja existe...';
    	else
    		echo 'Adicionando Repositorio: Astah Community...';sleep 1
    		apt-key adv --keyserver pgp.nic.ad.jp --recv-keys C22BD678;echo "deb http://members.change-vision.com/repos/apt/ astah main" > /etc/apt/sources.list.d/astah.list
    fi

# Sublime Text Editor
    if [ -e /etc/apt/sources.list.d/webupd8team-sublime-text-2-trusty.list ];  then
...
#Atom Text Editor
    if [ -e /etc/apt/sources.list.d/webupd8team-atom-trusty.list ];  then
...
# Ubuntu Make (Ferramentas de desenvolvimento)   
    if [ -e ubuntu-desktop-ubuntu-make-trusty.list ];  then
...
if [ -e /etc/ocsinventory-agent/ocsinventory-agent.cfg ];  then
	echo "OCS Agent já instalado e configurado"
	else
		#scp administrador@10.0.0.4:/home/administrador/Ocsinventory-Unix-Agent-2.1.1.tar.gz .
		cp $REPO/Ocsinventory-Unix-Agent-2.1.1.tar.gz .
		...
		make install
		cd ..
		rm -rf Ocsinventory-Unix-Agent-2.1.1/ Ocsinventory-Unix-Agent-2.1.1.tar.gz
fi
#What is the address of your ocs server?> http://10.0.0.5/ocsinventory
